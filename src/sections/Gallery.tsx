...
const ProjectCard = ({ project, index }: ProjectCardProps) => {
  ...
  return (
    <motion.div
      ...
      className={`relative group overflow-hidden bg-surface rounded-sm ${
        isLarge
    ...
    <section id="work" ref={containerRef} className="py-32 md:py-48 px-6 md:px-12 bg-surface/30">
      <div className="container mx-auto max-w-7xl">
        <div className="mb-20 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            <h2 className="text-sm font-semibold tracking-[0.3em] uppercase text-accent mb-4">
              Selected Work
            </h2>
        ...
        <div className="mb-8 flex gap-4">
          {['All', 'Residential', 'Commercial', 'Retail', 'Hospitality'].map((c) => (
            <button
              key={c}
              onClick={() => setFilter(c)}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-colors hover-target ${
                filter === c ? 'bg-accent text-white' : 'glass text-primary'
              }`}
            >
              {c}
            </button>
